...
impl PacketRelayer {
    pub fn new() -> Self { Self { misestimations: Vec::new() } }

    pub fn check_timeout_accuracy(&mut self, packet_id: u64, expected_timeout_height: u64, destination_height: u64) -> bool {
        if destination_height >= expected_timeout_height { return true; }
        self.misestimations.push(PacketTimeoutMisestimation { packet_id, expected_timeout_height, actual_destination_height: destination_height });
        false
    }

    pub fn misestimation_count(&self) -> usize { self.misestimations.len() }
